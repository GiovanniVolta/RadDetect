...
int analysis(){
	...
	int spec_range[2] = {150, 400};
	int fit_ranges[2] = {160, 350};
	...
	std::vector<double> cut_deadtime;
	std::vector<double> periodic_cut_start;
	double periodic_cut_width = 500;
	double periodic_cut_period = 24*60;

	isotope_labels.push_back("{}^{226}Ra");
	fit_isotope.push_back(true);
	isotope_color.push_back(Tableau20(0));
	tmp_line_pos.push_back(190);

	isotope_labels.push_back("{}^{222}Rn");
	fit_isotope.push_back(true);
	isotope_color.push_back(Tableau20(18));
	tmp_line_pos.push_back(220);

	isotope_labels.push_back("{}^{218}Po");
	fit_isotope.push_back(true);
	isotope_color.push_back(Tableau20(4));
	tmp_line_pos.push_back(240);

	isotope_labels.push_back("{}^{214}Po");
	fit_isotope.push_back(true);
	isotope_color.push_back(Tableau20(16));
	tmp_line_pos.push_back(320);
	...
	filenames.push_back("Mn15022021");
	run_labels.push_back("w/ top ring");
	min_runtime_cut.push_back(0);
	cut_deadtime.push_back(0);
	periodic_cut_start.push_back(820);
	filenames.push_back("Mn23022021");
	...
	std::vector<TCut> periodic_cut;
	for(unsigned int i = 0; i < trees.size(); ++i){
		TCanvas *c_time_evolution = new TCanvas(("evolution_"+std::to_string(i)).c_str());
		trees[i]->Draw(("channel:runtime>>h_time_"+std::to_string(i)+"(1000, 0, 12e3,"
				+std::to_string(n_bins)+","                     
                                +std::to_string(spec_range[0])+"," 
                                +std::to_string(spec_range[1])+")").c_str());
		TH2D* this_evolution = (TH2D*) c_time_evolution->GetPrimitive(("h_time_"+std::to_string(i)).c_str());	

		// Indicate the bad intervals, which are not usable for the analysis

		TCut this_run_cut;
		// for(int j = 0; j < 8; ++j){
		for(int j = 0; j < 5; ++j){

			double this_min_time = periodic_cut_start[i] + j * periodic_cut_period;
			double this_max_time = this_min_time + periodic_cut_width;

			if (j == 4){
			// Cut all away from day 5 on
			this_max_time = trees[i]->GetMaximum("runtime");
			}

			// if there was still data in this period, add this cut
			// interval to the deadtime
			if(trees[i]->GetMaximum("runtime") > this_max_time){
				cut_deadtime[i] += this_max_time - this_min_time;
			}
			else if(trees[i]->GetMaximum("runtime") < this_min_time){ 
				// The cut is not important at all
				continue;
			}
			else{
				// Partial fraction of data is in the cut
				cut_deadtime[i] += trees[i]->GetMaximum("runtime") - this_min_time;
			}

			TBox *this_cut = new TBox(this_min_time, spec_range[0],this_max_time,spec_range[1]);
			this_cut->SetFillColor(2);
			this_cut->SetFillStyle(3744);
			this_cut->Draw("SAME");
			TCut this_period_tcut = ("(runtime>"+std::to_string(this_min_time)+
						 ")&&(runtime<"+std::to_string(this_max_time)+")").c_str();
			this_run_cut = this_run_cut || this_period_tcut;

			
			
		}
		periodic_cut.push_back(this_run_cut);
		this_evolution->SetTitle(run_labels[i].c_str());
		this_evolution->GetYaxis()->SetTitle("channel");
		this_evolution->GetXaxis()->SetTitle("runtime (min)");
	}


//TESTING
TCanvas *c_test = new TCanvas("evolution_a");
trees[0]->Draw("channel:runtime", !periodic_cut[0]);

TCanvas *c_test_2 = new TCanvas("evolution_b");
trees[1]->Draw("channel:runtime", !periodic_cut[1]);


// Bin counting
TCut bin_range = "(channel>160)&&(channel<207)";
	...
	for(unsigned int i = 0; i < trees.size(); ++i){
		...
		trees[i]->Draw(("channel>>h_spec_"+std::to_string(i)+"("
				+std::to_string(n_bins)+","
				+std::to_string(spec_range[0])+","
				+std::to_string(spec_range[1])+")").c_str(), !periodic_cut[i]);
		TH1D* this_spec = (TH1D*) c_overview->GetPrimitive(("h_spec_"+std::to_string(i)).c_str());
		spectra.push_back(this_spec);
		double this_runtime = trees[i]->GetMaximum("runtime");
		// Subtract the interval cut deattime
		this_runtime = this_runtime - cut_deadtime[i];
		runtimes.push_back(this_runtime);
		double bin_counting_entries = trees[i]->GetEntries( !periodic_cut[i] + bin_range);

		std::cout<<"Runtime ("<<i<<"): "<<this_runtime<<std::endl;
		std::cout<<"Detected rate 226Ra: ("<<bin_counting_entries / this_runtime / 60 * 1e3<<
			   " +/- "<<sqrt(bin_counting_entries)/this_runtime / 60 * 1e3<<") mHZ"<<std::endl;
	}

	TCanvas *c_norm = new TCanvas("c2");
	c_norm->SetLogy();	
	...
	for(unsigned int i = 0; i < spectra.size(); ++i){
		...
		for(unsigned int j = 0; j < N_ISOTOPES_FIT; ++j){
			...
			int area_idx = j*N_PAR_CB;
                        fit_f->SetParameter(area_idx, 5000);
			fit_f->SetParLimits(area_idx, 1E3, 1E4);
			// Alphas are par[1]
			...
			fit_f->SetParLimits(mean_idx, 0.95*tmp_line_pos[j], 1.05*tmp_line_pos[j]);
			// Sigmas are par[3]:
			int sigma_idx = j*N_PAR_CB + 3;
                        fit_f->SetParameter(sigma_idx, 4);
			fit_f->SetParLimits(sigma_idx, 1, 15);
			// Ns are par[4]:
...
*/
	...
	spectra[0]->GetYaxis()->SetTitle("Detected activity [mBq]");
	spectra[0]->GetYaxis()->SetRangeUser(0.005, 4E1);
	gStyle->SetOptStat(0);
	spectra[0]->SetTitle("");
