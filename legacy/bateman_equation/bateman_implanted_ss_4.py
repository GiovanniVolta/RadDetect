...
for i in range(len(file_names)):
	...
	po212_selection = all_po212_selection[i]
	...
	# Find bins that are (partially) contained in a cutted region
	del_bins_idx = []
	for this_cut in time_cuts:
		this_cut = this_cut + start_times[0]
		print(this_cut)
		for i_bin in range(len(bins)):
			this_bin_edges_l = bins-bins_width/2
			this_bin_edges_h = bins+bins_width/2
			if this_bin_edges_h[i_bin] < this_cut[0] or this_bin_edges_l[i_bin] > this_cut[1]:
				# This bin is completely before or after the cut interval
				continue
			else:
				# This bin is at least parially inside the cut => mark it for removal
				del_bins_idx.append(i_bin)
	print(del_bins_idx)
	bins = np.delete(bins, del_bins_idx)
...
bin_centers_cut = bin_centers
bin_counts_cut = bin_counts
...
ax1.plot(np.array((t_1,t_1))/3600./24, (-1, 5), '--',label=r"$\mathrm{T_1}$(coating)")
...
ax1.set_title("224Ra implanted stainless steel sample #3")

ax1.set_ylim(5e-5,15)
ax1.set_yscale('log')
ax2.set_ylim(-5,5)
...
plt.show()
plt.savefig('implanted_ss_4_2.png')
