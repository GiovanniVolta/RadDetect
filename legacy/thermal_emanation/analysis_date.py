...
file_names = ['Ap28012020', 'Ap31012020', 'Ap04022020_preheatin', 'Ap04022020', 'Ap07022020', 'Ap11022020_preheatin', 'Ap11022020', 'Ap17022020_cooling', 'Ap17022020']
temperatures = np.array((60., 20., -1, 100., 20, -1, 145, -1, 20.))
colors = ["gold", "white", "gray", "orange", "white", "gray", "red", "gray", "white"] 
...
for i in range(len(file_names)):
	...
	min_timestamp = np.min(selected_timestamps)
	max_timestamp = np.max(selected_timestamps)
	interval=[]
	for i in range(0,len(selected_timestamps)):
		interval.append(selected_timestamps[i])
	bin_conts, bin_edges = np.histogram(interval, bins=bins)
	...
	if temperatures[i] > 20:
		plt.axvspan(0, t_max -t_min, alpha=0.25, color=colors[i], label='{:3.0f}'.format(temperatures[i]) +r'$^\circ$C')
	else:
		plt.axvspan(0, t_max - t_min, alpha=0.5, color=colors[i])
