...
int comparison(){
	...
	run_labels.push_back("irina_a");
	legend_entry.push_back("coated");
	irina_a.push_back("Rn05102017");
        irina_a.push_back("Rn16102017");
	filenames.push_back(irina_a);
	channel_cuts.push_back(std::make_pair(380,500));
	channel_cuts_bi.push_back(std::make_pair(270,310));

	n_bins.push_back(10);
	colors.push_back(2);

	std::vector<std::string> irina_b;
        run_labels.push_back("irina_b");
        legend_entry.push_back("H2SO4");
        irina_b.push_back("Rn27102017");
        filenames.push_back(irina_b);
        channel_cuts.push_back(std::make_pair(380,500));
        channel_cuts_bi.push_back(std::make_pair(285,310));
	n_bins.push_back(25);

        colors.push_back(3);

        std::vector<std::string> irina_c;
        run_labels.push_back("irina_c");
        legend_entry.push_back("un-coated");
        irina_c.push_back("Rn23012018");
        irina_c.push_back("Rn02022018");
        filenames.push_back(irina_c);
        channel_cuts.push_back(std::make_pair(390,500));
        channel_cuts_bi.push_back(std::make_pair(270,310));
        //n_bins.push_back(25);
	n_bins.push_back(50);
        ...
        std::vector<std::string> irina_d;
        run_labels.push_back("irina_d");
        legend_entry.push_back("de-coated");
        irina_d.push_back("Rn21032019");
        irina_d.push_back("Rn22032019");
	irina_d.push_back("Rn01042019");
        filenames.push_back(irina_d);
        channel_cuts.push_back(std::make_pair(270,330));
        channel_cuts_bi.push_back(std::make_pair(185,215));
	//n_bins.push_back(25);
        n_bins.push_back(42);	
        colors.push_back(6);
...
//			if (((this_channel > min_channel)&&(this_channel < max_channel)))
		...
		TF1 *fitFunc;
		if (legend_entry[i] != "de-coated_dummy"){
			fitFunc = new TF1(("212PoFit"+std::to_string(i)).c_str(), FitFunc212Po,  -5E3, 20E3, 2);
		}else{
			fitFunc = new TF1(("212PoFit"+std::to_string(i)).c_str(), FitFunc224Ra,  -5E3, 20E3, 2);
		}
...
*/
	...
	histos[0]->GetXaxis()->SetRangeUser(0,30000);
        histos[0]->GetYaxis()->SetTitle("Po212 Activity (mBq)");
