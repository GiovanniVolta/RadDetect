...
t0 = 0
runs_uncoated = ['Rn22022021', 'Rn28022021']
t1 = 6.745
runs_coated = ['Rn01032021', 'Rn07032021']
t2 = 13.75
runs_decoated = ['Rn10032021']

# define time intervals to cut
time_cuts = np.array((	(0.17, 1.81),
			(6.8, 7.2),
			(8.85, 9.8),
			(11.2, 12.85),
			(13.78,16.51),
...
x_axis_lim = np.array([-0.5, 21.3])

if plot_corrected_rate:
    y_axis_act_lim = np.array([0.002, 14])
else:
...
# reject bins whose center are falling into an excluded interval
time_interval_mask = np.zeros_like(bin_centers)
for jdx in range(np.shape(time_cuts)[0]):
	time_interval_mask += np.logical_and(np.greater(bin_centers-t_min, time_cuts[jdx,0]),
                                             np.less(bin_centers-t_min, time_cuts[jdx,1]))
time_interval_mask = np.logical_not(time_interval_mask)
print(time_interval_mask)

interval_rejection_mask = interval_rejection_mask * time_interval_mask
...
a224_0 = [p_opt[0], np.sqrt(p_cov[0,0])]
a224_1 = [p_opt[2], np.sqrt(p_cov[2,2])]
a224_2 = [p_opt[3], np.sqrt(p_cov[3,3])]
...
indicator_box_y = [y_axis_act_lim[0], 0.004]
indicator_box_color = ["gray", "darkgray", "lightgray"]
indicator_box_text = ["Before coating", "After coating", "Coating removed"]  
...
ax1.text(t1-arrow_offset_1-text_offset, np.sqrt(a224_0_zero*a224_1_zero), "$\mathrm{R_{max}}$", 
...
ax1.text(t2+arrow_offset_2-text_offset, np.sqrt(a224_1_zero*a224_2_zero), "$\mathrm{R_{min}}$", 
         ha='center', va='center', rotation=90, size=25+2, fontweight='semibold')
ax1.annotate("", xy=(t2+arrow_offset_2, a224_1_zero), xytext=(t2+arrow_offset_2,a224_2_zero),
            arrowprops=dict(facecolor=arrowcol, width=arrowwidth,linewidth=0), 
            ha='center', zorder=2)
...
if plot_corrected_rate:
    ...
    ax1.hlines(a224_1_zero, t1-arrow_offset_1-text_offset, 
               t2+arrow_offset_2+text_offset, colors=act_line_cols[1], zorder=1,
               linewidth=act_line_width, linestyle=act_line_style) 
...
# indicate regions excluded from the data
for this_time_cut in time_cuts/3600./24:
	ax1.fill_between(	this_time_cut, 2*[ax1.get_ylim()[0]], 2*[ax1.get_ylim()[1]],
				facecolor='none', edgecolor='gray', hatch='//', zorder=1)
...
plt.savefig("implanted_ss_4_initial_ra224_1.pdf",dpi="figure")
plt.show()
