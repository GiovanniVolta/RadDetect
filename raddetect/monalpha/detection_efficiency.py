...
class MonAlphaDetectorGeometricalEfficiency:
    ...
    def __init__(self, detector_settings):
        ...
        self.DIST_SAMP_DIODE = (
            geometry["dist_diode_flange"]
            - geometry["dist_flange_samp"]
            + geometry["uncertainty_dist_samp_diode"]
        )
    ...
    def generate_positions(self):
        ...
        if self.IMP_DISTR == "point":
            pos_z = (
                np.ones((self.N_IONS))
                * -1
                * (self.IMPL_DEPTH * 1e-6 + self.DIST_SAMP_DIODE)
            )
        elif self.IMP_DISTR == "gauss":
            pos_z = np.random.normal(
                loc=-1 * (self.IMPL_DEPTH * 1e-6 + self.DIST_SAMP_DIODE),
                scale=self.IMPL_SPREAD * 1e-6,
                size=self.N_IONS,
            )
    ...
    def calculate_intersections(self, positions, directions):
        ...
        vec_d = np.stack(
            (np.ones((self.N_IONS)), np.zeros((self.N_IONS)), np.zeros((self.N_IONS))),
            axis=1,
        )
        vec_e = np.stack(
            (np.zeros((self.N_IONS)), np.ones((self.N_IONS)), np.zeros((self.N_IONS))),
            axis=1,
        )

        solutions = self.calculate_plane_intersection(
            positions, directions, vec_c, vec_d, vec_e
        )

        # Get the intersection
        intersections = (
            positions
            + np.array((solutions[:, 0], solutions[:, 0], solutions[:, 0])).T
            * directions
        )
    ...
    def check_diode_hit(self, intersections):
        ...
        if self.DIODE_SHAPE == "square":
            bool_diode_hit = np.logical_and(
                ...
                np.less(np.abs(intersections[:, 1]), self.DIODE_SIZE / 2.0),
            )
        elif self.DIODE_SHAPE == "round":
            bool_diode_hit = np.less(
                np.sqrt(intersections[:, 0] ** 2 + intersections[:, 1] ** 2),
                self.DIODE_SIZE / 2.0,
            )
    ...
    def run_simulation(
        ...
        self.intersections = self.calculate_intersections(
        ...
        self.detection_efficiency = (
            self.calculate_detection_efficiency(self.bool_diode_hit) / 2
        )
        self.detection_efficiency_stat_uncert = (
            self.calculate_detection_efficiency_stat_uncertainty(self.bool_diode_hit)
            / 2
        )
